...
/**
 滚动到指定的的日期
 */
-(instancetype)initWithDateStyle:(WSDateStyle)datePickerStyle each10MinsProgress: (BOOL)each10Mins scrollToDate:(NSDate *)scrollToDate CompleteBlock:(void(^)(NSDate *))completeBlock {
    self = [super init];
    if (self) {
        self = [[[NSBundle mainBundle] loadNibNamed:NSStringFromClass([self class]) owner:self options:nil] lastObject];
        
        self.sureEach10MinsProgress = each10Mins;
        self.datePickerStyle = datePickerStyle;
...
-(void)defaultConfig {
    ...
    if (_sureEach10MinsProgress == YES) {
        [_minuteArray removeAllObjects];
        [_minuteArray addObject:@"0"];
